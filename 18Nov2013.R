...
Cand.models <- list( )
Cand.models[[1]] <- glm(Pos.Neg ~ Season + Year + Species + Sample.Type, family = binomial, data=data)
Cand.models[[2]] <- glm(Pos.Neg ~ Season + Sample.Type,family = binomial, data=data)
Cand.models[[3]] <- glm(Pos.Neg ~ Year + Sample.Type,family = binomial, data=data)
Cand.models[[4]] <- glm(Pos.Neg ~ Species + Sample.Type,family = binomial, data=data)
Cand.models[[5]] <- glm(Pos.Neg ~ Season + Year + Sample.Type, family = binomial, data=data)
Cand.models[[6]] <- glm(Pos.Neg ~ Species + Season + Sample.Type,family = binomial, data=data)
Cand.models[[7]] <- glm(Pos.Neg ~ Species + Year + Sample.Type,family = binomial, data=data)


##create a vector of names to trace back models in set
Modnames <- c("GLM_1", "GLM_2", "GLM_3", "GLM_4","GLM_5", "GLM_6" , "GLM_7")
