...
Based on a tutorial prepared from 1#https://botany.natur.cuni.cz/hodnocenidat/Lesson_05_tutorial.pdf
I just used and modified it for my ddradseq analysis
...
vcf <- read.vcfR("data/NEW3.recode2.vcf")   #read in all data 
...
```{r}
mySum <- glSum(aa.genlight.z, alleleAsUnit = TRUE)  
...
m1<-glm(PC1log~RUN+island+YEAR, data= meta)
m2<-glm(PC1log~island+YEAR, data= meta)
m3<-glm(PC1log~RUN+YEAR, data= meta)
m4<-glm(PC1log~RUN+island, data= meta)
m5<-glm(PC1log~RUN, data= meta)
m6<-glm(PC2log~island, data= meta)
m7<-glm(PC1log~YEAR, data=meta)
